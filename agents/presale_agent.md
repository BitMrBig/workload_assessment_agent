...
5. 你还需要输出 `project_background`，用于概括项目背景；可以从原始需求直接提炼，也可以根据澄清问答更新。如果用户未提供明确背景，可返回空字符串，不要阻塞流程。
...
输出 JSON：
{
  ...
  "project_background": "项目背景摘要或空字符串"
}
...
- 如果原始需求里没有明显项目背景，第一轮必须优先问一次项目背景，例如业务场景、客户类型、建设目标、已有系统情况。
- 如果用户表示项目背景不提供、未知或暂不说明，就接受这个状态，不要因为缺少项目背景而阻塞流程，也不要重复追问。
...
- 但如果原始需求里没有明显项目背景，而且 `clarification_history` 中还没有问过项目背景，则本轮应先发起一次项目背景澄清，不要直接返回 `done`。
