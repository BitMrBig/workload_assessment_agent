...
- 澄清问答会整理成摘要给评估岗位参考，项目背景也会整理成摘要供评估参考
...
3. 如果信息不足，CLI 进入澄清问答，直到达到可评估状态或超过最大澄清轮次。项目背景可以询问，但不是必填，用户不提供也不会阻塞流程。
4. 澄清完成后，CLI 展示当前模块清单及对应功能描述，请用户确认。
5. 用户如果提出“新增、删除、修改、补充”要求，仍然交给 `presale_agent` 处理，更新模块列表和描述后再次展示确认。
6. 澄清问答会整理成摘要；项目背景如果存在，也会整理成摘要。
7. 只有当 `presale_agent` 判断用户已经明确确认模块清单及描述后，才继续进入评估流程。
8. `dispatcher_agent` 提供岗位建议，但所有岗位都会参与评估。
9. 各岗位 Agent 会结合最细模块、模块描述、澄清摘要和项目背景摘要输出估算，最后合并为 Markdown 报告、Excel 和完整会话记录。
